...
custom_df = pd.read_csv('V1_JDK/CustomArrayDeque_performance.csv', sep=';')
jdk_df = pd.read_csv('ArrayDeque_performance.csv', sep=';')

# Define outlier methods to exclude from the main comparative visualization
# (You can showcase these separately in a callout box or text summary)
excluded_outliers = {
...
# Extract method names, ignoring 'Size' and any specified outliers
methods = [col for col in custom_df.columns if col != 'Size' and col not in excluded_outliers]
...
plt.tight_layout()
plt.savefig('deque_core_performance_chart_transparent.png', dpi=300, transparent=True)
plt.close()

print("Clean core-operations chart generated successfully without outlier skew!")